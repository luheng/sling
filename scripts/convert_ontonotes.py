    ...
    for line in json_input.readlines():
      ...
      for sent_id, sentence in enumerate(doc_info['sentences']):
        ...
        for i, frame in enumerate(predicates):
          if frame != '-':
            none_v_args = [a for a in srl_info if (
                a[0] == token_id_offset+i and a[3] != 'V')]
            if none_v_args or not FLAGS.remove_v_args:
              ...
              print ('Skipping non-argument-bearing predicate: ' + sentence[i])
